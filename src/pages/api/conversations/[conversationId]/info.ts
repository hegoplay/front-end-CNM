...
export default async function handler(
  ...
  try {
    ...
    const conversationName = fields.conversationName?.[0] || "";
    ...
    // Xử lý baseImg
    const baseImgFile = Array.isArray(files.baseImg)
      ? files.baseImg[0]
      : files.baseImg;
    if (baseImgFile && baseImgFile.size > 0 && baseImgFile.filepath) {
      ...
      formData.append(
        "baseImg",
        fileBlob,
        baseImgFile.originalFilename || "baseImg"
      );
    }
    ...
    const backendResponse = await fetch(
      `${apiUrl}/conversations/${conversationId}/info`,
      {
        method: "PUT",
        body: formData,
        headers: {
